"""Repeated-CV theory-guided VTSS analysis for retrospective sepsis."""
...
def main():
    ...
    print(
        f"Starting sepsis TG-VTSS: {N_SPLITS} outer train/test splits",
        flush=True,
    )
    ...
    if missing:
        raise KeyError(f"Missing required columns in sepsis.csv: {missing}")
    if df["subject_id"].duplicated().any():
        raise ValueError("sepsis.csv must contain only one admission per subject_id.")
    ...
    with open(SCRIPT_DIR / "sepsis_tg_vtss_outputs.pkl", "wb") as handle:
        pickle.dump(outputs, handle, protocol=pickle.HIGHEST_PROTOCOL)
    long, summary = summarize_metrics(outputs)
    long.to_csv(SCRIPT_DIR / "sepsis_tg_vtss_metrics.csv", index=False)
    summary.to_csv(SCRIPT_DIR / "sepsis_tg_vtss_summary.csv", index=False)
    ...
    for generator_name in GENERATOR_NAMES:
        ...
        ax.set_title(f"Sepsis: {generator_name}", fontsize=16)
        ax.legend(fontsize=10)
        fig.tight_layout()
        slug = generator_name.lower().replace(" ", "_")
        fig.savefig(SCRIPT_DIR / f"sepsis_{slug}_tg_vtss.pdf", bbox_inches="tight")
        fig.savefig(SCRIPT_DIR / f"sepsis_{slug}_tg_vtss.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
