"""Repeated-CV theory-guided VTSS analysis for retrospective mortality."""
...
def main():
    ...
    print(
        f"Starting mortality TG-VTSS: {N_SPLITS} outer train/test splits",
        flush=True,
    )
    ...
    if missing:
        raise KeyError(f"Missing required columns in mortality.csv: {missing}")
    if df["subject_id"].duplicated().any():
        raise ValueError("mortality.csv must contain only one admission per subject_id.")
    ...
    with open(SCRIPT_DIR / "mortality_tg_vtss_outputs.pkl", "wb") as handle:
        pickle.dump(outputs, handle, protocol=pickle.HIGHEST_PROTOCOL)
    long, summary = summarize_metrics(outputs)
    long.to_csv(SCRIPT_DIR / "mortality_tg_vtss_metrics.csv", index=False)
    summary.to_csv(SCRIPT_DIR / "mortality_tg_vtss_summary.csv", index=False)
    ...
    for generator_name in GENERATOR_NAMES:
        ...
        ax.set_title(f"Mortality: {generator_name}", fontsize=16)
        ax.legend(fontsize=10)
        fig.tight_layout()
        slug = generator_name.lower().replace(" ", "_")
        fig.savefig(SCRIPT_DIR / f"mortality_{slug}_tg_vtss.pdf", bbox_inches="tight")
        fig.savefig(SCRIPT_DIR / f"mortality_{slug}_tg_vtss.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
