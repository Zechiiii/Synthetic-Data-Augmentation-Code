...
Let **n₀ − n₁** be the class-balance gap, where n₀ is the majority sample size and n₁ is the minority sample size:

| Regime | Default candidate sizes |
| --- | --- |
| Local symmetry | round(γ × (n₀ − n₁)), for γ ∈ {0, 0.05, 0.10, 0.15, 0.20} |
| Asymmetry, realistic generator | 80%-120% neighborhoods around the balance target and the directionally adjusted theory target |
| Asymmetry, potentially biased generator | round(γ × (n₀ − n₁)), for γ = 0, 0.05, …, 2.00 |
...
- Centered squared-loss model in dimension d = 5.
- Exact local-symmetry null.
- Imbalance settings include n₀ / n₁ = 20.
- Minority sizes n₁ ∈ {50, 250, 500}.
...
- majority distribution: Normal(0, I₂);
- true minority distribution: Normal((1, 0), I₂);
- biased aligned synthetic distribution: Normal((0.5, 0), I₂);
- imbalance ratio 20;
- minority sizes 6 through 3,200;
- 5,000 fresh test observations per class;
- 100 Monte Carlo repetitions.

It compares naive balancing—adding n₀ − n₁ synthetic observations—with the bias-canceling choice of adding 4 × (n₀ − n₁). The latter drives parameter error and balanced excess risk toward zero; balancing approaches a nonzero floor.
...
- minority sizes n₁ ∈ {10, 100, 200, 300, 400, 500}, with imbalance ratio 20;
- 100 repetitions;
...
The main mean-shift study uses dimension d = 20, n₁ = 100, n₀ = 2,000, 20 multipliers over the interval [0, 4], and 100 repetitions. Noise families are uniform cube, Rademacher, and uniform sphere. Generators are Oracle, SMOTE (five neighbors), Gaussian-fit, and Semi-Oracle.
...
**Linear configuration:** dimension d = 5, n₁ = 200, n₀ = 4,000; Gaussian majority, two-component Gaussian-mixture minority; SMOTE plus logistic regression; multipliers from 0 to 2 in increments of 0.05; balanced test set of 10,000 per class.

**Nonlinear configuration:** dimension d = 5 with the same class counts; non-Gaussian majority mixture and noisy-ring minority; bootstrap plus Gaussian jitter; logistic regression on 100 random Fourier RBF features; five-fold diagnostics and validation; balanced test set of 10,000 per class.
...
A nonlinear problem in dimension d = 10 with 800 majority and 100 minority observations compares:
